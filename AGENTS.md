...
- **`test agent`** *(optional)* — generate or refresh tests in `agent/test/` that verify the implementation matches the spec, then run them. Follow `spec/test.md`. This is the **only** invocation that touches `agent/test/` — bootstrap and update agent never write, run, or mention tests.
