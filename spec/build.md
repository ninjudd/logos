...
- `agent-sdk` — **agent-sdk** ([github.com/ExProtos/agent-sdk](https://github.com/ExProtos/agent-sdk)). One API over four runtimes (Claude Agent SDK, Codex AppServer, OpenAI Agents SDK, Vercel AI SDK). Provides `Agent`, `agent.run()`, the canonical tool catalog (`bash`, `read`, `write`, `edit`, `glob`, `grep`, `webFetch`, `webSearch`, `todo`, `task`), and `withImpls()` for plugging in-process executors on backends that consult `execute`. agent-sdk handles the tool execution loop, automatic compaction, and per-backend session storage; do not implement a tool loop or token guard.
  ...
  - **Refresh agent-sdk on every `update` invocation.** `npm install` against a GitHub URL pins the resolved commit in `package-lock.json` and reuses it forever — re-running `npm install` does **not** pull in new agent-sdk commits. The `update` command must explicitly run `npm install github:ExProtos/agent-sdk@latest` (or `npm update agent-sdk`) inside `agent/` and commit the lockfile change. **Symptom of forgetting:** the spec describes a feature (e.g. typed `askForApproval` on the codex backend) but the generated agent code is reaching for a CLI workaround (`-c approval_policy=never` on `args:`). That's a stale agent-sdk — refresh first, regenerate the code path second.
...
1. **Refresh agent-sdk first.** Run `npm install github:ExProtos/agent-sdk@latest` (or `npm update agent-sdk`) inside `agent/` and stage the resulting `package-lock.json` change. agent-sdk is on a GitHub URL, not a registry version range, so npm pins the resolved commit forever otherwise — see Key packages → Refresh on every `update`. Skipping this step is the most common reason a spec change fails to reach the running agent.
