...
- **Enter key behavior:**
  ...
  - **Shift+Enter or Alt+Enter:** insert a literal `\n` into readline's **single input buffer** at the cursor position. The multi-line message lives in one editable buffer — the user can arrow back to earlier lines and edit them, same as any other text in the buffer. On plain Enter, the whole buffer (newlines and all) is sent in one `{type: "message"}` payload. Implementation: intercept the raw byte sequences before readline's submit path (Alt+Enter arrives as `ESC\r`; Shift+Enter as the kitty-keyboard-protocol sequence `ESC[13;2u`, which iTerm2 and similar terminals can be configured to emit). Splice `\n` into `rl.line` at `rl.cursor`, advance `rl.cursor` past it, then call `rl._refreshLine()` to redraw. These are private readline APIs — the surface is small (three touchpoints) and has been stable in Node for years. Leave a comment at the call site noting the dependency so it's findable if Node ever changes it.
