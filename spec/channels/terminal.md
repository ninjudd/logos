...
- **Input history (↑ / ↓).** The client keeps a history of previously sent messages. Up-arrow scrolls back through prior entries (most recent first), down-arrow scrolls forward toward the empty prompt. `node:readline` supports this natively via its `history` buffer — the client just needs to seed/maintain it as messages are sent. History persists across sessions, stored at `runtime/clients/{session}.history` (one entry per line, most recent last). Cap at a sensible size (e.g. 500 entries); trim the oldest when exceeded. Don't record blank lines or the `/quit` / `/exit` commands.
