...
- **On Enter (before sending):** the user's typed line is visible on the prompt line (`❯ hello?`), possibly wrapped across multiple terminal rows for long input. The client then **clears every row the input occupied** (compute from prompt width + input length + terminal width, then emit one cursor-up + clear-line per row) so the line can be redrawn uniformly by the render path when the server echoes it back. Clearing only the final row leaves the wrapped prefixes on screen and the echoed message appears *in addition to* the leftover fragments — giving a cascade of `❯ ...` on long input. Otherwise the same message appears twice — once as raw typed input, once as the rendered echo.
